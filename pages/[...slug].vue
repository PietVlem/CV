  ...
  <main class="container mx-auto pt-14 flex flex-col gap-8 md:gap-16 px-4 md:px-0">
    <div class="flex flex-col md:flex-row justify-between gap-4">
      ...
      <div class="flex flex-col justify-end">
        <div class="flex gap-4">
          <AppSocialButton icon="ph:github-logo-bold" url="https://github.com/PietVlem" />
          <AppSocialButton icon="ph:linkedin-logo-bold" url="https://www.linkedin.com/in/pieter-vleminckx-961663151/" />
        </div>
      </div>
    </div>
    ...
    <div v-if="data.bio" class="grid grid-cols-3 gap-16">
      <div class="col-span-full md:col-span-1 flex flex-col gap-4 md:gap-8">
        ...
        <div v-html="data?.bio?.bio" class="text-gray-500 text-sm flex flex-col gap-4"></div>
        <AppDataTable :data="data?.bio?.meta" />
        <a href="mailto:pieter.vlem@gmail.com"
      ...
      <div class="col-span-full md:col-span-2 flex flex-col gap-8">
        <div>
          ...
          <AppEductionRow v-for="(education, index) in data?.education" :key="`eduction-${index}`"
            :degree="education.degree" :school="education.institution" :startYear="education.start"
            :endYear="education.end" :description="education.description" />
        </div>
        <div class="mb-20">
          <AppSubHeading :title="data?.experienceTitle" />
          <AppExperienceRow v-for="(experience, index) in data?.experience" :key="`experience-${index}`"
